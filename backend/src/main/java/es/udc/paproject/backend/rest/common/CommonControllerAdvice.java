...
public class CommonControllerAdvice {
	...
	private final static String LATE_FOR_THE_SESSION_EXCEPTION_CODE = "project.exceptions.LateForTheSessionException";
	private final static String INVALID_DATE_EXCEPTION = "project.exceptions.InvalidDateException";
	...
	@ExceptionHandler(LateForTheSessionException.class)
	@ResponseStatus(HttpStatus.FORBIDDEN)
	@ResponseBody
	public ErrorsDto handleLateForTheSessionException(LateForTheSessionException exception, Locale locale) {

		String nameMessage = messageSource.getMessage(exception.getDate().toString(), null,
				exception.getDate().toString(), locale);
		String errorMessage = messageSource.getMessage(LATE_FOR_THE_SESSION_EXCEPTION_CODE,
				new Object[] { nameMessage }, LATE_FOR_THE_SESSION_EXCEPTION_CODE, locale);
	...
	public ErrorsDto handleInvalidDateException(InvalidDateException exception, Locale locale) {
		...
		String nameMessage = messageSource.getMessage(exception.getDate().toString(), null,
				exception.getDate().toString(), locale);
		String errorMessage = messageSource.getMessage(INVALID_DATE_EXCEPTION, new Object[] { nameMessage },
				INVALID_DATE_EXCEPTION, locale);
